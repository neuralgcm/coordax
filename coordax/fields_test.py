...
class FieldTest(parameterized.TestCase):
  ...
  def test_sel_supports_multidim_coords_in_indexers(self):
    x = coordax.LabeledAxis('x', np.arange(10))
    p = coordax.LabeledAxis('p', np.array([100, 200, 300, 500, 600]))
    p_to_sel = coordax.LabeledAxis('p', np.array([100, 300, 600]))
    px = coordax.coords.compose(p, x)
    field = coordax.field(jnp.zeros((5, 10)), px)
    px_sel = coordax.coords.compose(p_to_sel, x)
    f_sel = field.sel({px: px_sel})
    self.assertEqual(f_sel.dims, ('p', 'x'))
    self.assertEqual(f_sel.axes['x'], x)
    self.assertEqual(f_sel.axes['p'], p_to_sel)
