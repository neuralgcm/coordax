...
class Field:
  ...
  def isel(
    ...
    Examples:
      ...
      <Field dims=('x', 'y') shape=(2, 3) axes={} >
    """
    indexers = coordinate_systems.normalize_indexers(indexers, **indexers_kwargs)
    if not indexers:
      return self
  ...
  def sel(
    ...
    """
    sel_indexers = coordinate_systems.normalize_indexers(indexers, **indexers_kwargs)
    if not sel_indexers:
      return self

    mapped_indexers, consumed = self.coordinate.map_indexers(
        sel_indexers, method=method
    )
    unused_sel_indexers = set(sel_indexers.keys()) - consumed
