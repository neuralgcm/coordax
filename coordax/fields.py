...
def cpmap(
  ...
  """Coordinate preserving cmap, alias for `cmap(out_axes='same_as_input')`.
  ...
  Examples:
    ...
    >>> cx.op_cmap(lambda x: x.sum(axis=0))(field).dims
    ('x', 'y')

    `op_cmap` requires all inputs to have the same named axes ordering.
    The following will raise a ValueError:
    >>> a = cx.wrap(jnp.ones((2, 3)), 'x', 'y')
    >>> b = cx.wrap(jnp.ones((3, 2)), 'y', 'x')
    >>> # cx.op_cmap(jnp.add)(a, b)  # Raises ValueError
  """
